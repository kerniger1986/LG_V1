...
export function PersonaHero({
  ...
  phone,
  large,
  showRegionPhoto = false,
}: PersonaHeroProps) {
  const textBlock = (
    <div className={showRegionPhoto ? "" : "max-w-3xl"}>
  ...
  return (
    <section className="bg-[#2F5D50] text-[#EFF2ED]">
      <div
        className={`mx-auto max-w-5xl px-6 ${large ? "py-20 sm:py-24" : "py-16 sm:py-20"}`}
      >
        {showRegionPhoto ? (
